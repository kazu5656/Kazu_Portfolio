...
if ($_SERVER['REQUEST_METHOD'] != 'POST') {
        ...
        if (empty($err)) {
                    ...
                    // 差出人
                    $mailfrom="From:" .mb_encode_mimeheader("kazu-personaltraining") ."";

                    // 管理者にメールを送信"
                    mb_language("japanese");
                    mb_internal_encoding("UTF-8");
                    $mail_title = '【パーソナルトレーニング】受講希望がありました。';
                    $mail_body = '氏名：'. $user_name.PHP_EOL;
                    $mail_body.= '場所：'.$pt_place.PHP_EOL.'メールアドレス：'.$user_email.PHP_EOL.'電話番号：'.$user_phone_number.PHP_EOL.'ご質問：'.$Question;
                    mb_send_mail(kazu_mail, $mail_title, $mail_body, $mailfrom);

                    // 申込完了メールを送信"
                    mb_language("japanese");
                    mb_internal_encoding("UTF-8");
                    $mail_title = 'パーソナルトレーニングの受講希望のご連絡ありがとうございます。';
...
<head>
		...
		<title>PT 申し込みフォーム | <?php echo SERVICE_NAME; ?></title>
      ...
      <div class="row">
          <div class="col-md-12">
            ...
            <div class="section-heading left white">
                ...
                <p>パーソナルトレーニングについて<br class="br-携帯短" />何かご質問があれば、ご遠慮なく<br class="br-携帯短" />お問い合わせください。<br>
                  ...
                  <form id="demo_form"method="POST" class="panel panel-default panel-body" >
                    ...
                    <div class="form-group <?php if ($err['pt_place'] != '') echo 'has-error'; ?>">
                                <label id="受講場所">受講場所</label><br>
                                <?php echo arrayToSelect("pt_place", $pt_place_array, $pt_place); ?>
                                <span class="help-block"><?php echo h($err['pt_place']); ?></span>
                    </div>
                  ...
                  <a href="privacy_policy_pt_start.php"><input type="" value="プライバシーポリシーを確認する" class="btn btn-info btn-block"></a>
