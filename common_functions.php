...
echo "$filename $line\n";
    ...
    error_log(date("Y/m/d H:i:s")."$filename $line"." で\n500 Internal Server Errorが発生しました。\n".$e->getMessage(), 1, "goalhunter.kazu@gmail.com");
...
function logging($filename, $line, $message) {
    ...
    error_log(date("Y/m/d H:i:s")." ".$message."[".$filename.":".$line."]".PHP_EOL, 3, 'http://kazu0520.sakura.ne.jp/tablet_order_system2/logs/error_log');
...
function autoLogoutCheck ($login_table, $user_id,$pdo) {
        ...
        if (((strtotime('now') - strtotime($user3['last_action_time']))/60) > 2) {
...
// メールアドレスからuserを検索する
function getadminByEmail($user_email, $pdo) {
        $sql = "SELECT * FROM admin WHERE user_email = :user_email LIMIT 1";
        $stmt = $pdo->prepare($sql);
        $stmt->execute(array(":user_email" => $user_email));
        $user = $stmt->fetch();
        return $user ? $user : false;
...
// twitter_user_idからuserを検索する
function getUserBytwitter_user_id($twitter_user_id, $pdo) {
        $sql = "SELECT * FROM user WHERE twitter_user_id = :twitter_user_id LIMIT 1";
        $stmt = $pdo->prepare($sql);
        $stmt->execute(array(":twitter_user_id" => $twitter_user_id));
        $user = $stmt->fetch();
        return $user ? $user : false;
}
...
// メールアドレスの存在チェック(新規登録用）
function checkEmailadmin($user_email,$pdo) {
        $sql = "select * from admin where user_email = :user_email  limit 1";
        $stmt = $pdo->prepare($sql);
        $stmt->execute(array(":user_email" => $user_email));
        $user = $stmt->fetch();
...
function checkToken() {
        ...
        error_log(date("Y/m/d H:i:s")." 403 Error Forbiddenが発生しました。".PHP_EOL, 3, "../logs/error_log");
    error_log(date("Y/m/d H:i:s")." 403 Error Forbiddenが発生しました。", 1, "goalhunter.kazu@gmail.com");
